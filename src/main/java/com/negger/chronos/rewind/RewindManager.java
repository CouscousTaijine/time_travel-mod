...
public class RewindManager {
    ...
    public static void onReturnToPresent(ServerPlayerEntity player) {
        ...
        // On se contente d'armer le mode ici : la progression se fait pas à pas
        // dans tickAll(), comme pour les autres modes (HELD_BACKWARD, AUTO_TO_TARGET).
        // Avant, cette méthode rejouait tout le buffer d'un coup, en synchrone, dans
        // le même tick serveur : pour un rewind profond (plusieurs heures/jours
        // d'historique), ça pouvait bloquer le thread serveur assez longtemps pour
        // déclencher le watchdog et crasher le serveur. Le "tap = timelapse" annoncé
        // dans la doc n'était d'ailleurs même pas respecté puisque tout se jouait en un
        // seul tick au lieu d'être étalé dans le temps.
