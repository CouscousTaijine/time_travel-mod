...
public class RewindManager {
    ...
    public static boolean onRightClickPress(ServerPlayerEntity player) {
        ...
        p.startTick = HistoryManager.getCurrentTick(); p.sneaking = sneaking; PENDING.put(id, p);
        return true;
    }
    ...
    public static void onRightClickRelease(ServerPlayerEntity player) {
        ...
        if (p == null) return;
        if (!p.promoted) {
            if (p.sneaking) setSavepoint(player); else onReturnToPresent(player);
            return;
        }
        Session session = SESSIONS.get(id);
        if (session != null) finishAtPast(player, session);
        else { HistoryManager.setPaused(id, false); sendSmoothPacket(player, false); }
    }
    ...
    private static Session getOrCreateSession(ServerPlayerEntity player) {
        ...
        s = new Session(); s.buffer = buffer; s.cursor = buffer.size() - 1;
        SESSIONS.put(id, s); HistoryManager.setPaused(id, true);
        return s;
    }
    ...
    public static void onReturnToPresent(ServerPlayerEntity player) {
        ...
        if (s == null) return;
        BlockChange change;
        while ((change = HistoryManager.redoMatchingBlockChange(player.getUuid(), Long.MAX_VALUE)) != null) applyBlockChange(player, change, false, false);
    ...
    private static boolean advanceOneStep(ServerPlayerEntity player, Session session) {
        ...
        if (backward) {
            BlockChange c;
            while ((c = HistoryManager.popMatchingBlockChange(player.getUuid(), newTick + 1)) != null) applyBlockChange(player, c, true, true);
        } else {
            BlockChange c;
            while ((c = HistoryManager.redoMatchingBlockChange(player.getUuid(), newTick)) != null) applyBlockChange(player, c, false, true);
        }
        session.cursor = next;
    ...
    private static void applySnapshot(ServerPlayerEntity player, TimeSnapshot s, boolean smooth) {
        ...
        player.setYaw(s.yaw()); player.setPitch(s.pitch()); player.setVelocity(0, 0, 0); player.fallDistance = 0;
        var props = player.getServerWorld().getLevelProperties();
        props.setTimeOfDay(s.worldTime()); props.setRaining(s.raining()); props.setThundering(s.thundering());
        props.setClearWeatherTime(s.clearWeatherTime()); props.setRainTime(s.rainTime()); props.setThunderTime(s.thunderTime());
        if (ChronosConfig.restoreHealthAndHunger) {
            player.setHealth(Math.max(0.1f, Math.min(player.getMaxHealth(), s.health())));
            player.getHungerManager().setFoodLevel(s.foodLevel()); player.getHungerManager().setSaturationLevel(s.saturation());
        }
        player.experienceLevel = s.experienceLevel(); player.totalExperience = s.totalExperience(); player.experienceProgress = s.experienceProgress();
    ...
    private static void restoreEntities(ServerPlayerEntity player, long targetTick) {
        ...
        for (Entity entity : nearby) {
            ...
            if (snapshot == null) { entity.discard(); continue; }
            restoreEntity(entity, snapshot); touched.add(entity.getUuid());
        }
    ...
    private static void restoreEntity(Entity entity, EntitySnapshot snapshot) {
        try { entity.readNbt(snapshot.nbt().copy()); entity.setVelocity(0, 0, 0); }
        catch (Exception ignored) { entity.refreshPositionAndAngles(snapshot.x(), snapshot.y(), snapshot.z(), snapshot.yaw(), snapshot.pitch()); }
    }

    private static void applyBlockChange(ServerPlayerEntity player, BlockChange change, boolean reverse, boolean animate) {
        BlockPos pos = change.pos(); ServerWorld world = player.getServerWorld(); var state = reverse ? change.oldState() : change.newState();
    ...
    private static void sendSmoothPacket(ServerPlayerEntity player, boolean active) {
        ...
        buf.writeDouble(player.getX()); buf.writeDouble(player.getY()); buf.writeDouble(player.getZ()); buf.writeFloat(player.getYaw()); buf.writeFloat(player.getPitch()); buf.writeBoolean(active);
