...
public class HistoryManager {
    ...
    public static void recordSnapshot(ServerPlayerEntity player) {
        ...
        ServerWorldProperties props = world.getLevelProperties();
    ...
    public static List<TimeSnapshot> combinedHistory(UUID id) {
        List<TimeSnapshot> shortTerm = snapshotHistory(id), longTerm = getLongTermHistory(id);
        ...
        result.addAll(shortTerm); return result;
    }
    ...
    public static void truncateHistoryTo(UUID id, int keepCount) {
        Deque<TimeSnapshot> h = PLAYER_HISTORY.get(id); if (h == null) return;
        List<TimeSnapshot> list = new ArrayList<>(h); h.clear();
    ...
    public static BlockChange popMatchingBlockChange(UUID id, long minTick) {
        ...
        synchronized (h) {
            BlockChange c = h.peekLast(); if (c == null || c.tick() < minTick) return null;
            h.pollLast(); BLOCK_UNDO_STACK.computeIfAbsent(id, x -> new ArrayDeque<>()).addLast(c); return c;
        }
    }
    ...
    public static BlockChange redoMatchingBlockChange(UUID id, long maxTick) {
        ...
        synchronized (undo) {
            BlockChange c = undo.peekLast(); if (c == null || c.tick() > maxTick) return null;
            undo.pollLast(); BLOCK_HISTORY.computeIfAbsent(id, x -> new ArrayDeque<>()).addLast(c); return c;
        }
    }
